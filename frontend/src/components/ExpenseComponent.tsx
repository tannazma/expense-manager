...
const ExpenseComponent = ({ refetchBalance }: expenseProps) => {
  ...
  const getExpenseSum = async () => {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_SERVERURL}/accounts/${selectedAccountId}/expenses-sum?from=${dateFilter.from}&to=${dateFilter.to}`,
      {
        headers: {
  ...
  return (
    <FilterContext.Provider
      value={{ dateFilter, setDateFilter: setDateFilterState }}
    >
